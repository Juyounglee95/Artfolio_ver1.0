        ...
        switch (requestCode) {
            case PICK_FROM_GALLERY:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    Toast.makeText(this, "사진고르기를 취소하였습니다.", Toast.LENGTH_SHORT).show();
                }
                break;
        ...
        switch (requestCode) {
            case MY_PERMISSION_CAMERA:
                for (int i = 0; i < grantResults.length; i++) {
                    ...
                    if (grantResults[i] < 0) {
                        Toast.makeText(com.example.user.artfolio_ver10.getprofile.this, "해당 권한을 활성화 하셔야 합니다.", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
                // 허용했다면 이 부분에서..
